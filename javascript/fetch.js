...
async function fetchResponse(url, request) {
    ...
    try {
        ...
        // Den her catch fanger exceptions der bliver throwet af enten fetch i linje 23, eller .json() metoden i linje 26.
...
async function fetchAny(fetchUrl, fetchMethod, objectBody) {
    ...
    if (fetchedObject instanceof Error) {
        throw new Error(fetchedObject.message) // Den her error skal catches der hvor fetchAny metoden bliver kaldt. Dvs den skal i en try/catch
    }
...
// Hvis du skal poste noget data fra en form, skal du gemme form elementet i en variabel, og så kalde den her metode og give formen som parametre.
// Den data man får tilbage skal så gemmes i en variabel du så poster med fetchAny.
